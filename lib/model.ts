...
export function computeMetrics(design: DesignVector, _scenario: ScenarioSettings = defaultScenario): Metrics {
  ...
  const fixedLength = Math.max(0, totalLength - movableLength);

  const movableCostPerMeter = 5.2e5 + 4.8e4 * Math.pow(x2, 1.5) + 2.2e5 / minClosing;
  const permanentCostPerMeter = 1.8e4 + 5.5e3 * Math.pow(x2, 1.1);
  const initialCost = movableLength * movableCostPerMeter + fixedLength * permanentCostPerMeter;
