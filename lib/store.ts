...
function computeSeedFromState(
  paradigm: 'minmax' | 'tetra',
  weights: number[],
  objectiveWeights: number[][]
) {
  const key = `${paradigm}|${weights.map((w) => w.toFixed(6)).join(',')}|${objectiveWeights
    .flat()
    .map((w) => w.toFixed(6))
    .join(',')}`;
  let hash = 2166136261;
...
export const useMoseStore = create<StoreState>((set, get) => ({
  ...
  runGA: (paradigm) => {
    ...
    setTimeout(() => {
      ...
      const seed = computeSeedFromState(paradigm, state.stakeholderWeights, state.stakeholderObjectiveWeights);
