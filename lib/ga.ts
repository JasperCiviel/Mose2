...
function randomValue(random: () => number, min: number, max: number) {
...
export function runGeneticAlgorithm({
  ...
  const random = createRandom(seed ?? 1_234_567_89);
  ...
  const randomDesign = (): DesignVector => ({
    x1: randomValue(random, bounds.x1.min, bounds.x1.max),
    x2: randomValue(random, bounds.x2.min, bounds.x2.max),
    x3: randomValue(random, bounds.x3.min, bounds.x3.max)
  });

  const blend = (a: number, b: number) => a + (b - a) * random();
  ...
  for (let iteration = 0; iteration < iterations; iteration += 1) {
    ...
    while (newPopulation.length < popSize) {
      ...
      const parentB = population[Math.floor(random() * population.length)];
      ...
      if (random() < 0.6) {
        childDesign.x1 = clamp(
          childDesign.x1 +
            (random() - 0.5) * mutationScale * (bounds.x1.max - bounds.x1.min),
          bounds.x1.min,
          bounds.x1.max
        );
      }
      if (random() < 0.6) {
        childDesign.x2 = clamp(
          childDesign.x2 +
            (random() - 0.5) * mutationScale * (bounds.x2.max - bounds.x2.min),
          bounds.x2.min,
          bounds.x2.max
        );
      }
      if (random() < 0.6) {
        childDesign.x3 = clamp(
          childDesign.x3 +
            (random() - 0.5) * mutationScale * (bounds.x3.max - bounds.x3.min),
          bounds.x3.min,
          bounds.x3.max
        );
      }
